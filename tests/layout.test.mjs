...
const port = server.address().port, modulePath = process.env.PLAYWRIGHT_MODULE, executablePath = process.env.BROWSER_EXECUTABLE;
if (!modulePath || !executablePath) { console.log('layout browser test skipped: set PLAYWRIGHT_MODULE and BROWSER_EXECUTABLE to run it.'); server.close(); process.exit(0); }
...
try {
  ...
  for (const [width, height] of sizes) {
    await page.setViewportSize({ width, height }); await page.goto(`http://127.0.0.1:${port}/`, { waitUntil:'networkidle' });
  ...
  await page.setViewportSize({ width:1440, height:900 }); await page.goto(`http://127.0.0.1:${port}/`, { waitUntil:'networkidle' }); await page.setViewportSize({ width:320, height:568 }); await page.setViewportSize({ width:1440, height:900 });
