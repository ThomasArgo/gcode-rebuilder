...
try {
  ...
  for (const [width, height] of sizes) {
    ...
    const check = await page.evaluate(() => { const viewer = document.querySelector('#viewer'), canvas = viewer.querySelector('canvas'), box = viewer.getBoundingClientRect(), canvasBox = canvas.getBoundingClientRect(); return { pageFits:document.documentElement.scrollWidth <= document.documentElement.clientWidth, canvasFits:canvasBox.width <= box.width + 1 && canvasBox.height <= box.height + 1, status:document.querySelector('.status-message')?.textContent.trim(), sourceLink:[...document.links].some(link => /github\.com\/ThomasArgo\/gcode-rebuilder/.test(link.href)), noHeaderNav:!document.querySelector('.app-header nav'), width:box.width, height:box.height }; });
    if (!check.pageFits || !check.canvasFits || !check.width || !check.height || check.status !== 'Load a G-code file to begin.' || check.sourceLink || !check.noHeaderNav) throw new Error(`${width}x${height}: ${JSON.stringify(check)}`);
  }
